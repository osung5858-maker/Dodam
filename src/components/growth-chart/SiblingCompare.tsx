    ...
    return (
      <div className="bg-white rounded-2xl border border-[#D5D0CA] shadow-sm overflow-hidden">
        <div className="px-4 py-3 flex items-center justify-between border-b border-[#E8E4DF]">
          <p className="text-[14px] font-bold text-[#1A1918]">비교 히스토리</p>
          <button onClick={() => setShowHistory(false)} className="text-[12px] text-[#6B6966]">닫기</button>
        </div>
        <div className="max-h-[300px] overflow-y-auto">
          {history.length === 0 ? (
            <p className="text-[13px] text-[#9E9A95] text-center py-8">아직 비교 기록이 없어요</p>
          ) : (
            history.map((entry, i) => (
              <button
                key={i}
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[13px] font-semibold text-[#1A1918]">{entry.ageMonths}개월 비교</p>
                    <p className="text-[11px] text-[#9E9A95]">{entry.date}</p>
                    <p className="text-[11px] text-[#6B6966] mt-0.5 line-clamp-1">{entry.insight.insight.slice(0, 50)}...</p>
                  </div>
  ...
  if (!insight && !loading) {
    return (
      <div className="bg-gradient-to-br from-[#F5F0FF] to-[#FFF8F3] rounded-2xl border border-[#E0D8EF] p-5 text-center">
        <div className="flex justify-center gap-1 mb-2"><BabyIcon className="w-6 h-6 text-[#9E9A95]" /><BabyIcon className="w-6 h-6 text-[#9E9A95]" /></div>
        <p className="text-[15px] font-bold text-[#1A1918] mb-1">형제자매 AI 인사이트</p>
        <p className="text-[13px] text-[#6B6966] mb-1">
          {currentChild.name}과(와) {siblings.map(s => s.name).join(', ')}의
        </p>
        <p className="text-[13px] text-[#6B6966] mb-4">성장을 비교해볼까요?</p>
        <button onClick={fetchInsight} className="px-5 py-2.5 bg-[var(--color-primary)] text-white text-[13px] font-semibold rounded-xl active:opacity-80">
          비교 분석하기
        </button>
        {history.length > 0 && (
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-2xl border border-[#D5D0CA] shadow-sm p-5 text-center">
        ...
        <p className="text-[13px] text-[#6B6966]">형제자매 인사이트 분석 중...</p>
      </div>
    )
  }
    ...
    <div className="bg-white rounded-2xl border border-[#D5D0CA] shadow-sm p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          <BabyIcon className="w-4 h-4 text-[#9E9A95]" /><BabyIcon className="w-4 h-4 text-[#9E9A95]" />
          <h3 className="text-[13px] font-bold text-[#212124]">형제자매 AI 인사이트 · {ageMonths}개월</h3>
        </div>
      </div>

      <p className="text-[14px] text-[#4A4744] leading-relaxed">{insight!.insight}</p>

      {insight!.olderAtSameAge && (
        <div className="p-3 rounded-xl bg-[#FFF8F3] border border-[#F0E4D8]">
          <p className="text-[11px] text-[#9E9A95] mb-1">{siblings[0]?.name}이(가) 이 월령이었을 때</p>
          <p className="text-[13px] text-[#5A5854]">{insight!.olderAtSameAge}</p>
        </div>
      )}
        ...
        <div className="flex gap-2">
          {insight!.uniqueTraits.map((trait, i) => (
            <div key={i} className="flex-1 p-2.5 rounded-xl bg-[#F5F0FF] border border-[#E0D8EF]">
              <p className="text-[11px] text-[#7A6FA0] font-medium mb-0.5">
                {i === 0 ? currentChild.name : siblings[i - 1]?.name || ''}
              </p>
              <p className="text-[12px] text-[#5A5854]">{trait}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {insight!.funFact && (
        <div className="flex items-start gap-1.5">
          ...
          <p className="text-[13px] text-[var(--color-primary)] font-medium">{insight!.funFact}</p>
        </div>
      )}

      <div className="p-3 rounded-xl bg-[#F0F9F4] border border-[#D5E8DC]">
        <p className="text-[12px] text-[#2D7A4A]">{insight!.parentTip}</p>
      </div>

      <div className="flex gap-2 pt-1">
        <button onClick={fetchInsight} className="flex-1 py-2 text-[12px] text-[#6B6966] font-medium bg-[var(--color-page-bg)] rounded-lg active:bg-[#E8E4DF]">
          다시 분석
        </button>
        <button onClick={() => setShowHistory(true)} className="flex-1 py-2 text-[12px] text-[#6B6966] font-medium bg-[var(--color-page-bg)] rounded-lg active:bg-[#E8E4DF]">
          히스토리 ({history.length})
        </button>
        <button onClick={() => shareSiblingCompare(insight!.insight)} className="flex-1 py-2 text-[12px] text-[var(--color-primary)] font-semibold bg-[var(--color-page-bg)] rounded-lg active:bg-[#E8E4DF]">
          공유
        </button>
      </div>
    </div>
